...
void Parse::merge_common(Parse::Block* target, int pnum) {
  ...
  if (!target->is_merged()) {   // No prior mapping at this bci
    ...
    //tty->print_cr("Updateing mappings"); 
    ...
    for (uint j = 1; j < newin->req(); j++) {
          ...
          default:                // All normal stuff
            if (phi == nullptr) {
              ...
              if (EliminateNestedLocks &&
                ...
                //tty->print_cr("Merge with existing phi!");
      ...
      if (phi != nullptr) {
        ...
        if (n == top()) {
          //tty->print_cr("Node is typ: %d", j);
          //tty->print_cr("For: blk %d, target: %d, save_blk: %d", block()->rpo(), target->rpo(), save_block->rpo());
          //m->dump();
        }
        if (r->in(pnum) != top()) {
          //tty->print_cr("Region at end is not top :/");
          //r->in(pnum)->dump();
          //tty->print_cr("R is: ");
          //r->dump();
          //tty->print_cr("Control is: ");
          //control()->dump();
          if (save_r != nullptr){
            //tty->print_cr("Old control is: ");
            //save_r->dump();
          }
